		...
		if (message.type == 'observe') {
			...
			if (localStorage.length > 0) {
				if (disableds.indexOf(message.url) == -1) {
					chrome.contentSettings.javascript.get({ 'primaryUrl': message.url }, function(result) {
						port.postMessage({ 'type': 'storage', 'loop': false, 'storage': localStorage, 'browser': 'chrome', 'javascript': /allow/i.test(result.setting) });
					});
				}
			} else {
